...
// These functions shouldn't be required, in theory this should work
// with Rcpp::wrap. I have had no luck getting that to work, however. 
...
DenseRasterFlowSimulator::DenseRasterFlowSimulator(SEXP inputHeightMatrix,
                                                   ...
                                                   SEXP seed)
{
    ...
    // 4. current col
    // 5. group number
    currentMembership = Rcpp::IntegerMatrix(heightMatrix.nrow()*heightMatrix.ncol(), 
                                            5);    
    idx = 0;
    for (j = 0; j < membershipMatrix.ncol(); j++)
    {
        for (i = 0; i < membershipMatrix.nrow(); i++)
        {
            currentMembership(idx,0) = i;
            currentMembership(idx,1) = j;

            currentMembership(idx,2) = i;
            currentMembership(idx,3) = j;

            currentMembership(idx,4) = membershipMatrix(i,j);
            idx++;
        }
    }
    if (idx != (currentMembership.nrow()))
    {
        Rcpp::stop("currentMembership incorrectly populated.");
    }
...
DenseRasterFlowSimulator::~DenseRasterFlowSimulator()
{
    delete generator;
}

int DenseRasterFlowSimulator::getDescentDirection(int Xidx, int Yidx)
{
    int dir,l;
...
void DenseRasterFlowSimulator::simulate(int n)
{
...
Rcpp::IntegerMatrix DenseRasterFlowSimulator::getContactMatrix()
{
    ...
    for (i = 0; i < currentMembership.nrow(); i++)
    {
        ...
        cidx = (Yidx * originalHeightMatrix.nrow() 
                                            + Xidx);
        targetMemberNum = currentMembership(cidx, 4);
        output(sourceMemberNum - 1, targetMemberNum -1) += 1;
    }
    return(output);
}

Rcpp::IntegerMatrix DenseRasterFlowSimulator::getDensityMatrix()
{
...
Rcpp::IntegerMatrix DenseRasterFlowSimulator::getDensityMatrix2(SEXP inIndices)
{
...
RCPP_MODULE(DenseRasterFlowSimulator){
    using namespace Rcpp ;
    class_<DenseRasterFlowSimulator>( "DenseRasterFlowSimulator" )
    .constructor<SEXP, SEXP, SEXP, SEXP>()
    .method("simulate", &DenseRasterFlowSimulator::simulate)
    .method("simulateOCL", &DenseRasterFlowSimulator::simulate)
    .method( "getContactMatrix", &DenseRasterFlowSimulator::getContactMatrix )
    .method( "getDensityMatrix", &DenseRasterFlowSimulator::getDensityMatrix2 )
    .method( "getDensityMatrix", &DenseRasterFlowSimulator::getDensityMatrix );
